...
const VietMapNavigationScreen = ({ navigation, route }) => {
  ...
  const [latitude, setLatitude] = useState(""); // Giá trị vĩ độ nhập
  const [longitude, setLongitude] = useState(""); // Giá trị kinh độ nhập
  useEffect(() => {
    console.log("vietmapAPIKey: " + vietmapAPIKey);
    Geolocation.getCurrentPosition(
      (position) => {
        const { latitude, longitude } = position.coords;
        setCurrentLatLng({ lat: latitude, long: longitude });
        console.log("🚀 ~ useEffect ~ setCurrentLatLng:", currentLatLng);

        setIsLoading(false); // Ẩn trạng thái tải sau khi có tọa độ
      },
      (error) => {
  ...
  const handleCreateRoute = () => {
    ...
    if (!latitude || !longitude) {
      Alert.alert("Lỗi", "Vui lòng nhập đầy đủ vĩ độ và kinh độ!");
      return;
    }
    ...
    VietMapNavigationController.buildRoute(
      ...
      .then(() => {
        Alert.alert("Thành công", "Tuyến đường đã được tạo!");
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.inputContainer}>
        <TextInput
          style={styles.input}
          placeholder="Nhậpppppp vĩ độ (latitude)"
        ...
        <TouchableOpacity style={styles.button} onPress={handleCreateRoute}>
          <Text style={styles.buttonText}>Tạo tuyến đườngggg</Text>
        </TouchableOpacity>
      </View>
      {bannerInstruction}
      {bottomActionBar}
