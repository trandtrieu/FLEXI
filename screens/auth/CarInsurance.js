...
const CarInsurance = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchCarInsuranceData = async () => {
      try {
        const carInsuranceData = await AsyncStorage.getItem("vehicleInsurance");
        if (carInsuranceData) {
          const { frontImage, backImage } = JSON.parse(carInsuranceData);
  ...
  const handleSave = async () => {
    if (!frontImage || !backImage) {
      Alert.alert(
        "Thông báo",
        "Bạn cần tải lên cả hai mặt trước và sau của bảo hiểm xe."
      );
      return;
    }
    ...
    try {
      ...
      // Create the carInsurance data object
      const carInsuranceData = {
        frontImage: frontImageUrl,
        backImage: backImageUrl,
      };

      // Save carInsurance data to local storage
      await AsyncStorage.setItem("vehicleInsurance", JSON.stringify(carInsuranceData));
  ...
  return (
    ...
    >
      ...
      >
        <Text style={styles.headerText}>Tải lên bảo hiểm xe</Text>
