...
const CarImage = ({ navigation }) => {
  const [frontImage, setFrontImage] = useState(null);
  const [backImage, setBackImage] = useState(null);
  const [uploading, setUploading] = useState(false);

  // Fetch car image data from local storage when component mounts
  useEffect(() => {
    const fetchCarImageData = async () => {
      try {
        const CarData = await AsyncStorage.getItem("vehicleImages");
        if (CarData) {
          const { frontImage, backImage } = JSON.parse(CarData);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Create the car image data object
      const CarImageData = {
      ...
      await AsyncStorage.setItem("vehicleImages", JSON.stringify(CarImageData));
    ...
    >
      ...
      >
        ...
        <Text style={styles.subLabelText}>
          Thấy rõ nguyên thân xe và đầu xe
        </Text>
        <TouchableOpacity
        ...
        >
          <Text style={styles.cameraButtonText}>Chụp ảnh hình đầu xe</Text>
        </TouchableOpacity>

        <Text style={styles.labelText}>Hình đuôi xe (Bắt buộc)</Text>
        <Text style={styles.subLabelText}>Thấy rõ thân xe và biển số xe</Text>
        <TouchableOpacity
          style={styles.uploadButton}
        ...
        >
          <Text style={styles.cameraButtonText}>Chụp ảnh hình đuổi xe</Text>
        </TouchableOpacity>
